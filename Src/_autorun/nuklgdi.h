    ...
    short y1, unsigned int line_thickness, struct nk_color col)
{
    ...
    if (pen) {
        SelectObject(dc, GetStockObject(/*DC_PEN*/ BLACK_PEN));
        DeleteObject(pen);
    }
}
    ...
    unsigned short h, unsigned short line_thickness, struct nk_color col)
{
    ...
    HBRUSH brush = CreateSolidBrush(OPAQUE);
    HGDIOBJ oldBrush = SelectObject(dc, brush);
    Ellipse(dc, x, y, x + w, y + h);
    SelectObject(dc, oldBrush);
    ...
    const char *text, int len, GdiFont *font, struct nk_color cbg, struct nk_color cfg)
{
    ...
    wsize = MultiByteToWideChar(/*CP_UTF8*/ CP_ACP, 0, text, len, NULL, 0);
    wstr = (WCHAR*)_alloca(wsize * sizeof(wchar_t));
    MultiByteToWideChar(/*CP_UTF8*/ CP_ACP, 0, text, len, wstr, wsize);
...
nk_gdi_handle_event(HWND wnd, UINT msg, WPARAM wparam, LPARAM lparam)
{
    ...
    case WM_LBUTTONDOWN:
        gdi.mouseX = (short)LOWORD(lparam);
        gdi.mouseY = (short)HIWORD(lparam);
    ...
    case WM_CAPTURECHANGED:
        if (gdi.left) {
            nk_input_button(&gdi.ctx, NK_BUTTON_DOUBLE, gdi.mouseX, gdi.mouseY, 0);
            nk_input_button(&gdi.ctx, NK_BUTTON_LEFT, gdi.mouseX, gdi.mouseY, 0);
            gdi.left = 0;
        }
        if (gdi.right) {
            nk_input_button(&gdi.ctx, NK_BUTTON_RIGHT, gdi.mouseX, gdi.mouseY, 0);
            gdi.right = 0;
        }
        if (gdi.mid) {
            nk_input_button(&gdi.ctx, NK_BUTTON_MIDDLE, gdi.mouseX, gdi.mouseY, 0);
            gdi.mid = 0;
        }
        return 1;
    }
...
nk_gdi_render(struct nk_color clear)
{
    ...
    SelectObject(memory_dc, GetStockObject(/*DC_PEN*/ BLACK_PEN));
    SelectObject(memory_dc, GetStockObject(/*DC_BRUSH*/ WHITE_BRUSH));
